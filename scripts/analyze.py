...
with open(os.path.join(RESULTS_DIR, "results.csv")) as f:
    ...
    for r in reader:
        ...
        for k in ("lat_p50", "lat_p95", "lat_p99", "lat_p999"):
            try:
                r[k] = float(r[k])
...
tp_agg = defaultdict(list)
lat_agg = defaultdict(lambda: defaultdict(list))
for r in rows:
    key = (r["scheduler"], int(r["threads"]))
    tp_agg[key].append(r["throughput"])
    for p in ("lat_p50", "lat_p95", "lat_p99", "lat_p999"):
        if r[p] is not None:
            lat_agg[p][key].append(r[p])

schedulers = ["rr", "rs", "lqs", "as"]
colors = {"rr": "#1f77b4", "rs": "#ff7f0e", "lqs": "#2ca02c", "as": "#d62728"}
markers = {"rr": "o", "rs": "s", "lqs": "^", "as": "D"}
threads = sorted(set(k[1] for k in tp_agg))
...
for sched in schedulers:
    ...
    for t in threads:
        ...
        vals.append(speedup)
    ax2.plot(
        threads, vals, label=sched.upper(), color=colors[sched], marker=markers[sched]
    )
...
for sched in schedulers:
    ...
    try:
        ...
        ax.plot(
            threads, vals, color=colors[sched], marker=markers[sched], linestyle="none"
        )
        ax.plot(
            threads, fit, color=colors[sched], linestyle="--", label=label, alpha=0.7
        )
...
for sched in schedulers:
    vals = []
    for t in threads:
        data = lat_agg["lat_p99"][(sched, t)]
        vals.append(np.mean(data) / 1000.0 if data else 0)
    ax.plot(
        threads, vals, label=sched.upper(), color=colors[sched], marker=markers[sched]
    )
...
ax.set_title("P99 Latency vs Thread Count (lower is better)")
ax.legend()
ax.grid(True, alpha=0.3)
ax.set_yscale("log")
...
for sched in schedulers:
    row = []
    for t in threads:
        row.append(np.mean(tp_agg[(sched, t)]))
...
print("-" * 48)
for sched in schedulers:
    vals = [f"{np.mean(tp_agg[(sched, t)]):8.0f}" for t in threads]
...
for sched in schedulers:
    ...
    for t in threads:
        data = lat_agg["lat_p99"][(sched, t)]
        vals.append(f"{np.mean(data):8.0f}" if data else "     N/A")
    print(f"{sched.upper():>8} | " + " | ".join(vals))
...
for sched in schedulers:
    vals = []
    for t in threads:
        rr_val = np.mean(tp_agg[("rr", t)])
        s_val = np.mean(tp_agg[(sched, t)])
        vals.append(f"{s_val / rr_val:7.2f}x")
    print(f"{sched.upper():>8} | " + " | ".join(vals))

# --- Strong Scaling ---
sched_1t = {s: np.mean(tp_agg[(s, 1)]) for s in schedulers}

fig, ax = plt.subplots(figsize=(8, 5))
ax.plot(threads, threads, "--", color="gray", alpha=0.5, label="Ideal (p×)")
for sched in schedulers:
    vals = [np.mean(tp_agg[(sched, t)]) / sched_1t[sched] for t in threads]
    ax.plot(
        threads, vals, label=sched.upper(), color=colors[sched], marker=markers[sched]
    )
ax.set_xlabel("Worker Threads")
...
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(PLOTS_DIR, "scaling_strong.png"), dpi=150)
print("Saved scaling_strong.png")

# --- Weak Scaling (Efficiency) ---
fig, ax = plt.subplots(figsize=(8, 5))
ax.axhline(y=1.0, color="gray", linestyle="--", alpha=0.5, label="Ideal")
for sched in schedulers:
    vals = [np.mean(tp_agg[(sched, t)]) / (t * sched_1t[sched]) for t in threads]
    ax.plot(
        threads, vals, label=sched.upper(), color=colors[sched], marker=markers[sched]
    )
ax.set_xlabel("Worker Threads")
ax.set_ylabel("Efficiency E(p)")
ax.set_title("Weak Scaling — Parallel Efficiency vs Thread Count")
ax.legend()
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(PLOTS_DIR, "scaling_weak.png"), dpi=150)
print("Saved scaling_weak.png")

# --- Strong Scaling Speedup table ---
print("\n=== Strong Scaling Speedup ===")
hdr = [f"{t}T" for t in threads]
print(f"{'Scheduler':>8} | " + " | ".join(f"{h:>8}" for h in hdr))
print("-" * 48)
for sched in schedulers:
    vals = [f"{np.mean(tp_agg[(sched, t)]) / sched_1t[sched]:7.2f}x" for t in threads]
    print(f"{sched.upper():>8} | " + " | ".join(vals))
...
for sched in schedulers:
    vals = [
        f"{np.mean(tp_agg[(sched, t)]) / (t * sched_1t[sched]):7.3f}" for t in threads
    ]
    print(f"{sched.upper():>8} | " + " | ".join(vals))
